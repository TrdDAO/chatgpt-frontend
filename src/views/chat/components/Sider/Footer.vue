<script setup lang='ts'>
import { computed, defineAsyncComponent, ref } from 'vue';
import wechat from '@/assets/icons/wechat.svg';
import { useBasicLayout } from '@/hooks/useBasicLayout';
import { useAppStore, useSystemStore } from '@/store';
import { HoverButton, SvgIcon, UserAvatar } from '@/components';

const Setting = defineAsyncComponent(() => import('@/components/Setting/index.vue'));

const show = ref(false)

const appStore = useAppStore();
const systemStore = useSystemStore();
const { isMobile } = useBasicLayout();

systemStore.getSystemProfile();

const theme = computed(() => appStore.theme);

const wechatImagesrc = computed(() => {
  if(theme.value === 'dark'){
    return systemStore.getSystemWechatGroupQRDarkImage
  } else if(theme.value === 'light') {
    return systemStore.getSystemWechatGroupQRLightImage
  }
})
</script>

<template>
  <footer class="flex items-center justify-between min-w-0 p-4 overflow-hidden border-t dark:border-neutral-800">
    <div class="flex-1 flex-shrink-0 overflow-hidden">
      <UserAvatar />
    </div>

    <HoverButton tooltip="node" placement="top" :style="{padding: 0}" trigger="click">
      <span class="text-xl text-[#4f555e] dark:text-white">
        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 512 512" width="1em" height="1em"><path data-name="XMLID 501 -1" d="M408.67 298.53a21 21 0 1 1 20.9-21a20.85 20.85 0 0 1-20.9 21m-102.17 0a21 21 0 1 1 20.9-21a20.84 20.84 0 0 1-20.9 21m152.09 118.86C491.1 394.08 512 359.13 512 319.51c0-71.08-68.5-129.35-154.41-129.35s-154.42 58.27-154.42 129.35s68.5 129.34 154.42 129.34c17.41 0 34.83-2.33 49.92-7c2.49-.86 3.48-1.17 4.64-1.17a16.67 16.67 0 0 1 8.13 2.34L454 462.83a11.62 11.62 0 0 0 3.48 1.17a5 5 0 0 0 4.65-4.66a14.27 14.27 0 0 0-.77-3.86c-.41-1.46-5-16-7.36-25.27a18.94 18.94 0 0 1-.33-3.47a11.4 11.4 0 0 1 5-9.35" fill="currentColor"></path><path data-name="XMLID 505 -7" d="M246.13 178.51a24.47 24.47 0 0 1 0-48.94c12.77 0 24.38 11.65 24.38 24.47c1.16 12.82-10.45 24.47-24.38 24.47m-123.06 0A24.47 24.47 0 1 1 147.45 154a24.57 24.57 0 0 1-24.38 24.47M184.6 48C82.43 48 0 116.75 0 203c0 46.61 24.38 88.56 63.85 116.53C67.34 321.84 68 327 68 329a11.38 11.38 0 0 1-.66 4.49C63.85 345.14 59.4 364 59.21 365s-1.16 3.5-1.16 4.66a5.49 5.49 0 0 0 5.8 5.83a7.15 7.15 0 0 0 3.49-1.17L108 351c3.49-2.33 5.81-2.33 9.29-2.33a16.33 16.33 0 0 1 5.81 1.16c18.57 5.83 39.47 8.16 60.37 8.16h10.45a133.24 133.24 0 0 1-5.81-38.45c0-78.08 75.47-141 168.35-141h10.45C354.1 105.1 277.48 48 184.6 48" fill="currentColor"></path></svg>
      </span>
      <template #tooltip>
        <div class="w-[300px] bg-white dark:bg-[#24272e]">
          <img v-lazy="wechatImagesrc" class="mx-auto my-0"/>
        </div>
      </template>
    </HoverButton>

    <HoverButton @click="show = true">
      <span class="text-xl text-[#4f555e] dark:text-white">
        <SvgIcon icon="ri:settings-4-line" />
      </span>
    </HoverButton>

    <Setting v-if="show" v-model:visible="show" />
  </footer>
</template>
